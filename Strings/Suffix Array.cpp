struct SuffixArray {
    ...
    vector<int> sortCyclicShifts(const string& t) {
        ...
        for (int k = 0; (1 << k) < n; ++k) {
            ...
            for (int i = n - 1; i >= 0; --i) {
                position[--frequency[eClass[k][newPosition[i]]]] = newPosition[i];
            }
            eClass[k + 1][position[0]] = 0;
            classesCount = 1;
            for (int i = 1; i < n; ++i) {
                pair<int, int> current = {eClass[k][position[i]], eClass[k][(position[i] + (1 << k)) % n]};
                pair<int, int> previous = {eClass[k][position[i - 1]], eClass[k][(position[i-1] + (1 << k)) % n]};
                if (current != previous) {
                    ++classesCount;
                }
    ...
    // first occurrence of t in the sortedShifts array
    ...
    // last occurrence of t in the sortedShifts array
